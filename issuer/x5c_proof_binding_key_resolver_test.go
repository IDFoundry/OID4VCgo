...
func TestX5CProofBindingKeyResolver_AcceptsCASignedLeaf(t *testing.T) {
	...
	pub, err := resolver.ResolveProofBindingKey(context.Background(), x5cProofHeader(leaf))
	if err != nil {
...
func TestX5CProofBindingKeyResolver_RejectsMissingX5C(t *testing.T) {
	...
	if _, err := resolver.ResolveProofBindingKey(context.Background(), map[string]any{"kid": "k1"}); err == nil {
...
func TestX5CProofBindingKeyResolver_RejectsEmptyX5C(t *testing.T) {
	resolver := issuer.X5CProofBindingKeyResolver{Roots: x509.NewCertPool()}
	if _, err := resolver.ResolveProofBindingKey(context.Background(), map[string]any{"x5c": []any{}}); err == nil {
...
func TestX5CProofBindingKeyResolver_RejectsSelfSignedLeafEvenIfTrusted(t *testing.T) {
	...
	if _, err := resolver.ResolveProofBindingKey(context.Background(), x5cProofHeader(leaf)); err == nil {
