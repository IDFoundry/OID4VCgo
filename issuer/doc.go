...
// (§4), the Deferred Credential Endpoint (§9), and — as it lands — the
...
// by reference), and the Deferred Credential Endpoint's own polling
// protocol (§9) exist so far. See CredentialRequest's own doc comment
// for exactly what the Credential Endpoint doesn't implement yet
// (credential_identifier, di_vp, kid/x5c-based key resolution,
// request/response encryption, unbound credentials — and note it
// never defers issuance itself), CredentialOffer's own doc comment for
// what the Credential Offer doesn't cover yet (issuing/redeeming a
...
// to own that), DeferredTransactionRecord's own doc comment for why
// this package never creates or resolves a Deferred Issuance
// transaction itself, and ARCHITECTURE.md for the planned shape of
// what's still missing at the package level (the Notification
// Endpoint).
package issuer
