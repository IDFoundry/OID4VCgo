...
// (Appendix F.1): typ, alg (against ptc's own allow-list), the "jwk"
// header (see jwkHeaderKey), the JWS signature (self-consistency — a
// proof is signed by the very key it declares, proving possession),
// and the body's aud/iat/nonce claims. §8.2's own batch-issuance
// example shares one c_nonce across every proof in the request — this
// consumes it once, from the first proof, and requires every other
// proof to declare that same value, matching NonceStore's own doc
// comment ("Consume is still called once per request, not once per
// proof").
func (iss *Issuer) resolveJWTProofKeys(
	...
	for i, raw := range values {
		...
		if err != nil {
			return nil, newError(ErrorInvalidProof, 400, fmt.Sprintf("proof %d: parse jwk: %v", i, err), nil)
		}
