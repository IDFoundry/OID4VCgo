...
const (
	...
	// stores don't declare themselves adequate: every store
	// Dependencies actually needs (Nonces, CredentialOffers,
...
// checkProductionStoreAssurance runs checkStoreAssurance against every
// store New's own existing conditional-required checks have already
// confirmed is actually configured — called only once cfg.Assurance is
// known to be AssuranceProduction. Split out of New purely to keep its
// own cognitive complexity manageable.
func checkProductionStoreAssurance(cfg Config, deps Dependencies) error {
