...
// TestPreAuthorizedCodeStore_WrongTxCodeDoesNotConsume is the
// regression test for a real bug found in a repo-wide security
// review: the original implementation consumed (invalidated) the code
// unconditionally, then checked TxCode afterward — so a single
// mistyped PIN permanently destroyed the code with no retry path, a
// denial-of-service against the code's own legitimate holder. Consume
// must now leave the code usable after a wrong guess, and only
// actually invalidate it once the right TxCode is presented.
func TestPreAuthorizedCodeStore_WrongTxCodeDoesNotConsume(t *testing.T) {
	s := storage.NewPreAuthorizedCodeStore()
	ctx := context.Background()
	if err := s.Issue(ctx, "code-1", issuer.PreAuthorizedCodeRecord{
		TxCode: "493536", ExpiresAt: time.Now().Add(time.Minute),
	}); err != nil {
		t.Fatalf("Issue: %v", err)
	}
