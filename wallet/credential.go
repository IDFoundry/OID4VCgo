...
func (w *Wallet) RequestCredential(
	...
	if err != nil {
		return CredentialResult{}, fmt.Errorf("wallet: request credential: build request: %w", err)
	}
	...
	if err != nil {
		return CredentialResult{}, fmt.Errorf("wallet: request credential: read response: %w", err)
	}
